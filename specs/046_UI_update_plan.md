...
- **Visual Clutter:** Section headers in Settings are `text-base font-bold`, which adds unnecessary visual weight to a "compact" interface.
...
- **Technical Typography:** Reduce header sizes to `text-base font-bold uppercase tracking-wider` to match the "Quest/Solo Leveling" minimalist aesthetic without the bulk.
